...
class ClientThread extends Thread {
    ...
    private SGCRTRFrame _serverFrame;
    private Socket _clientSocket;
    SocketSGCRTR _socketServer;
    private String _type;
    private int _Id;
    public Camion _flightInfo;
    ...
    public ClientThread(SocketSGCRTR _socketServer) {
        try {
...
public class SocketSGCRTR extends Thread {

    public SGCRTRFrame _serverFrame;
    ...
    public SocketSGCRTR(SGCRTRFrame serverFrame, int port) {
        try {
            ...
            Logger.getLogger(SocketSGCRTR.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void HandleMessage(int threadId, Message msg) {
        ...
        if (msg.type.equals("flight")) {//msg received from pilot
            ...
            if (msg.command.contains("Connection")) {//Just an info
                _serverFrame.txtInfo.append("\n[" + msg.sender + "] >> [SGCRTR]: Connexion fermée..");
                if (  msg.sender == null )
                    return;
                int senderId ;
                try
    ...
    public List<Camion> GetFlights() {
        if (_flightThreads == null) {
            return null;
        }
        List<Camion> infoCollection = new ArrayList();
